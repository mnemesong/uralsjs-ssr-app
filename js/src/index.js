...
var nodeSsrRenderer = function (getElements, getRootSelector, elWidget, renderId, inputHtml, deps) {
...
var ssrApp = function (modelSets, html, deps) {
    ...
    Object.keys(state)
        ...
        html = nodeSsrRenderer(function () { return recs; }, modelSets[i].rootSelector, modelSets[i].widget, function (id) { return modelSets[i].idTool.renderId(id); }, html, deps);
    }); });
    Object.keys(state)
        .forEach(function (i) { return state[i].reinit(modelSets[i].initData(deps)); });
    return html;
};
